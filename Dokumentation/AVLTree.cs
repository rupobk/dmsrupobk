...
namespace DMSRupObk
{
    public class AVLTree<TKey, TValue> : IEnumerable<AVLNode<TKey, TValue>>
    {
        ...
        public double PageHeigth { get; set; }
        ...
        {
            return new AVLEnumerator<TKey, TValue>(_root);
        }
        ...
        public bool FindKey(TKey key, out AVLNode<TKey, TValue> result)
        {
            AVLNode<TKey, TValue> node = _root;
            while (node != null)
            {
        ...
        public bool FindAllSubTextChildren(TKey key, out List<TValue> result)
        {
            AVLNode<TKey, TValue> node;
            result = null;
            if (FindKey(key, out node))
            {
                result = new List<TValue>();
                result = GetAllSubTextChilden(key, node, result);
                if (result.Count > 0)
                    return true;
                else
                    return false;
        ...
        private List<TValue> GetAllSubTextChilden(TKey key, AVLNode<TKey, TValue> current, List<TValue> result)
        {
            if (current == null || !current.Key.ToString().Contains(key.ToString()))
                return result;

            result.Add(current.Value);
            result = result.Union(GetAllSubTextChilden(key, current.Right, result)).ToList<TValue>();
            return result;
        }
